...
import { View, Text, StyleSheet, Pressable, Switch, Platform, ActivityIndicator, AppState } from "react-native";
...
export default function WK() {
  ...
  useEffect(() => {
    setAudioModeAsync({ playsInSilentMode: true, shouldPlayInBackground: true, allowsRecording: true }).catch(() => {});
  }, []);
  ...
  // Reconnect when frequency changes while active
  useEffect(() => {
    if (active) connect();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [frequency]);
  ...
  const stopTalk = async () => {
    ...
    try {
      ...
      if (uri && wsRef.current?.readyState === WebSocket.OPEN) {
        ...
        wsRef.current.send(JSON.stringify({ type: "audio", audio_b64: b64, mime: "audio/m4a", from: user?.full_name || "?" }));
      }
    } catch {}
  };
      ...
      <View style={styles.body}>
        <View style={styles.statusCard}>
          ...
          <Text style={styles.statusText}>
            {active ? "Walkie attivo, tieni premuto per parlare" : "Walkie spento. Attivalo dall'interruttore in alto."}
          </Text>
        </View>
        {lastFrom ? (
          <View style={styles.lastFromCard}>
            <Ionicons name="volume-high" size={18} color={COLORS.brand} />
            <Text style={styles.lastFromText}>Ultimo messaggio da: <Text style={{ fontWeight: "700" }}>{lastFrom}</Text></Text>
          </View>
        ) : null}
        ...
        <Text style={styles.hint}>
          Premi e tieni premuto per parlare. Al rilascio il messaggio viene inviato a tutti sul canale.
        </Text>
      </View>
    </SafeAreaView>
  );
